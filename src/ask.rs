...
impl<M: Copy + Send + Sync + 'static + Reflect,
    E: Send + 'static>
...
pub trait AskPattern<Args: Copy + Send + Sync + 'static,
    M: Copy + Send + Sync + 'static + Reflect,
    A: Actor<M> + 'static,
...
AskPattern<Args, M, A, E> for ActorCell<Args, M, A> {
    fn ask<Message: Copy + Send + 'static + Reflect, T: CanReceive>(&self, to: T, message: Message) -> Future<M, E> {
